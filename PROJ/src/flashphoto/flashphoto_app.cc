...
const std::map<MBlurDir, std::string> FlashPhotoApp::mblur_dir_names_ = {
...
void FlashPhotoApp::InitNanoGUI() {
  ...
  b->setFixedSize({150, 30});
  b->setCallback([this] {
    image_editor_.ApplyMotionBlurFilter(mblur_radius_, mblur_dir_);
  });
  ...
  b->setFixedSize({150, 30});
  b->setCallback([this] {
    image_editor_.ApplySharpenFilter(sharpen_radius_);
  });
  ...
  b->setFixedSize({150, 30});
  b->setCallback([this] {
    image_editor_.ApplyChannelsFilter(chan_r_, chan_g_, chan_b_);
  });
