...
void PixelBuffer::SaveToFile(const std::string &filename) {
  ...
  for (int i=0; i < image->Width(); i++) {
    for (int j=0; j < image->Height(); j++) {
      ColorData current = pixel(i, j);
      image->SetFloatValue(i, j, 0, current.red());
      image->SetFloatValue(i, j, 1, current.green());
      image->SetFloatValue(i, j, 2, current.blue());
      image->SetFloatValue(i, j, 3, current.alpha());
    }
  }
...
void PixelBuffer::LoadFromFile(const std::string &filename) {
  if (mingfx::Platform::FileExists(filename)) {
    ...
    for (int i=0; i < image->Width(); i++) {
      for (int j=0; j < image->Height(); j++) {
        ColorData load = ColorData(image->FloatValue(i, j, 0),
           image->FloatValue(i, j, 1), image->FloatValue(i, j, 2),
           image->FloatValue(i, j, 3));
        set_pixel(i, j, load);
      }
    }
    delete image;
